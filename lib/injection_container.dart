...
Future<void> dependencyInjectionInit() async {
  //! Core
  sl.registerLazySingleton<NetworkInfo>(() => NetworkInfoImpl(connectionChecker: InternetConnectionChecker.instance));
  ...
  sl.registerLazySingleton<PhoneNumberValidatorService>(() => PhoneNumberValidatorServiceImp());

  // ! Payment Methods
  sl.registerSingleton<PaymentMethodsRDS>(PaymentMethodsRDSImpl(apiConsumer: sl<ApiConsumer>()));

  sl.registerSingleton<PaymentMethodsRepo>(PaymentMethodsRepoImpl(paymentMethodsRDS: sl<PaymentMethodsRDS>()));
