...
// Widgets
  Widget _buildPaymentMethods() {
    final bool isWalletSelected =
        widget.shouldHideOtherPaymentMethodsIfWalletSelected && widget.hasWalletPayment && _walletSwitchOn;
    ...
    return BlocBuilder<PaymentMethodsCubit, PaymentMethodsState>(
      builder: (context, state) {
        ...
        if (state.paymentMethods.isEmpty) return _buildErrorWidget(isEmpty: true);
        return Column(
          children: [
            showWallet
  ...
  Text _buildErrorWidget({required bool isEmpty}) {
    final String englishMessage = isEmpty ? 'No payment methods available' : 'Failed to load payment methods';
    final String arabicMessage = isEmpty ? 'No payment methods available' : 'Failed to load payment methods';
    return Text(
  ...
  Widget _buildWalletPaymentMethod(BuildContext context, PaymentMethod paymentMethod) {
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.umragold, borderRadius: BorderRadius.circular(4)),
                  child: Image.network(paymentMethod.image),
                ),
                SizedBox(
                  width: 14,
                ),
    ...
    return Column(
      ...
      children: [
        Text(
          ...
          style: fontStyle(fontSize: 24.sp, fontFamily: FontFamily.bold, fontWeight: FontWeight.w500),
        ),
        20.verticalSpace,
        widget.hasWalletPayment && widget.shouldHideOtherPaymentMethodsIfWalletSelected
            ? _buildWalletSwitch()
            : Container(),
      ],
    );
    ...
    return Row(
      children: [
        Text(
          ...
          style: fontStyle(color: Colors.black, fontSize: 21, fontFamily: FontFamily.medium),
        ),
        const Spacer(),
        CupertinoSwitch(
  ...
  onPaymentMethodPressed(PaymentMethod paymentMethod, BuildContext context) {
    final isVisa = paymentMethod.type == PaymentMethodType.creditCard && widget.onVisaPaymentPressed != null;
    final isFawry = paymentMethod.type == PaymentMethodType.fawry && widget.onFawryPressed != null;
    final isElectronicWallet =
        paymentMethod.type == PaymentMethodType.electronicWallet && widget.onElectronicWalletPressed != null;
