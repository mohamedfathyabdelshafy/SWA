...
class PaymentMethodsRDSImpl implements PaymentMethodsRDS {
  ...
  Future<List<PaymentMethod>> getPaymentMethods(
      {bool includeWallet = true}) async {
    var countryid = CacheHelper.getDataToSharedPref(
      key: 'countryid',
    );
    final res = await apiConsumer.get(
        "${EndPoints.baseUrl}Payment/GetPageList?countryID=$countryid&includWallet=$includeWallet");

    final decodedResponse = json.decode(res.body);
    final isSuccess = decodedResponse['state'] == 'success';
