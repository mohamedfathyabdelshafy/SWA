...
class _NotificationScreenState extends State<NotificationScreen> {
  ...
  Widget build(BuildContext context) {
    final _notificationBloc = BlocProvider.of<NotificationBloc>(context);
        ...
        body: BlocListener<NotificationBloc, NotificationState>(
          listener: (context, NotificationState state) {
            if (state.notificationModel?.status == 'success') {
              count = state.notificationModel?.notifications.where((notification) => !notification.IsRead!).length ?? 0;
            }
          },
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          InkWell(
                            ...
                            child: Text(
                              (LanguageClass.isEnglish ? 'Delete All' : 'حذف الكل'),
                              style: fontStyle(
                                  fontSize: 15,
                      ...
                      Container(
                        ...
                        child: Text(
                          LanguageClass.isEnglish ? 'Notifications' : 'الاشعارات',
                          style: fontStyle(
                      ...
                      Expanded(
                        child: state.notificationModel?.notifications != null &&
                            ...
                            ? ListView.builder(
                                itemCount: state.notificationModel!.notifications.length,
                                shrinkWrap: true,
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  return InkWell(
                                    onTap: () {
                                      NotifcationRespo()
                                          .readNotification(
                                              id: state.notificationModel?.notifications[index].NotificationID ?? 0)
                                          .then((value) {});
                                      state.notificationModel?.notifications[index].IsRead = true;
                                      ...
                                      if (state.notificationModel?.notifications[index].IsRead == true && count > 0) {
                                        count--;
                                      }
                                      setState(() {
                                    ...
                                    child: Dismissible(
                                      onDismissed: (direction) async {
                                        _notificationBloc.add(DeleteNotificationEvent(
                                            id: state.notificationModel!.notifications[index].NotificationID
                                                .toString()));
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                            ...
                                            color: state.notificationModel?.notifications[index].IsRead == false
                                                ? Color(0xffEBEBEB)
                                                : Colors.white),
                                        child: Row(
                                          children: [
                                            Stack(
                                              children: [
                                                state.notificationModel?.notifications[index].IsRead == false
                                                    ? Positioned(
                                                        top: 2,
                                                        child: badges.Badge(badgeContent: Container()),
                                                      )
                                                    : Container(),
                                                Icon(
                                                  Icons.notifications,
                                            ...
                                            Expanded(
                                              child: Text(
                                                state.notificationModel?.notifications[index].Description ?? "",
                            ...
                            : Center(
                                child: Text(
                                  (LanguageClass.isEnglish
                                      ...
                                      : 'ليس لديك أي إشعارات'),
                                  style:
                                      fontStyle(fontSize: 18, fontWeight: FontWeight.w600, color: AppColors.blackColor),
                                ),
                              ),
                      )
                    ],
