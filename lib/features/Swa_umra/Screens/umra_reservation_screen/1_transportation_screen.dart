...
class TransportationScreen extends StatefulWidget {
  ...
  TransportationScreen({super.key, required this.selectedpackage, required this.typeid, this.umrahReservationID});
...
class _TransportationScreenState extends State<TransportationScreen> {
  ...
  void initState() {
    ...
    _umraBloc.add(GetTransportationEvent(tripUmrahID: widget.typeid, reservationID: widget.umrahReservationID));
  }
        ...
        body: BlocListener(
          ...
          listener: (context, UmraState state) {
            if (state.campainlistmodel?.status == "success") {
              ...
              transportList = state.transportationListModel!.message!.transportList;
              transportSuggestedList = state.transportationListModel!.message!.transportSuggestedList;
              if (widget.umrahReservationID != null) {
                for (int i = 0; i < transportList!.length; i++) {
                  if (transportList![i].isreserved == true) {
                    BusLayoutRepo(apiConsumer: (sl()))
                        .getReservationSeatsData(reservationID: transportList![i].reservationId!)
                        .then((value) async {
                      ...
                      reservedseats.add(TransportationsSeats(
                          ...
                          totalprice: value.message.mySeatListId.length * transportList![i].priceSeat));
                      setState(() {});
                    });
                  }
                }
                for (int i = 0; i < transportSuggestedList!.length; i++) {
                  if (transportSuggestedList![i].isreserved == true) {
                    BusLayoutRepo(apiConsumer: (sl()))
                        .getReservationSeatsData(reservationID: transportSuggestedList![i].reservationId!)
                        .then((value) async {
                      log(value.message.mySeatListId.toString());
                      reservedseats.add(TransportationsSeats(
                          tripid: transportSuggestedList![i].tripId!,
                          seatsnumber: value.message.mySeatListId,
                          totalprice: value.message.mySeatListId.length * transportSuggestedList![i].priceSeat));
                      setState(() {});
                    });
                  }
                }
              ...
              var product = isSuggested
                  ? reservedseats.firstWhere(
                      (product) => product.tripid == transportSuggestedList![suggestedindex].tripId,
                      orElse: () => TransportationsSeats(
                            tripid: 0,
                            seatsnumber: [],
                            totalprice: 0,
                          ))
                  : reservedseats.firstWhere((product) => product.tripid == transportList![inn].tripId,
                      orElse: () => TransportationsSeats(tripid: 0, seatsnumber: [], totalprice: 0));
                                    ...
                                    onTap: () {
                                      if (isSuggested) {
                                        if (selectedseats.isNotEmpty) {
                                          transportList!.add(transportSuggestedList![suggestedindex]);
                                          transportSuggestedList!.removeAt(suggestedindex);

                                          if (reservedseats
                                                  .firstWhere((product) => product.tripid == transportList!.last.tripId,
                                                      orElse: () => TransportationsSeats(
                                            ...
                                            reservedseats
                                                .firstWhere(
                                                  (product) => product.tripid == transportList!.last.tripId,
                                                )
                                            ...
                                            reservedseats
                                                ...
                                                .totalprice = selectedseats.length * transportList!.last.priceSeat;
                                          } else {
                                            reservedseats.add(TransportationsSeats(
                                              seatsnumber: selectedseats,
                                              totalprice: (selectedseats.length * transportList!.last.priceSeat),
                                              tripid: transportList!.last.tripId!,
                                            ));
                                          }
                                        }
                                      } else {
                                        if (reservedseats
                                                .firstWhere((product) => product.tripid == transportList![inn].tripId,
                                                    orElse: () =>
                                                        TransportationsSeats(tripid: 0, seatsnumber: [], totalprice: 0))
                                                .tripid ==
                                          ...
                                          reservedseats
                                              ...
                                              .totalprice = selectedseats.length * transportList![inn].priceSeat;
                                        } else {
                                          reservedseats.add(TransportationsSeats(
                                            seatsnumber: selectedseats,
                                            totalprice: (selectedseats.length * transportList![inn].priceSeat),
                                            tripid: transportList![inn].tripId!,
                                          ));
                                        }
                                      }

                                      print(reservedseats.length.toString());
                                    ...
                                    Positioned(
                                      ...
                                      child: Container(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            for (int index = 0;
                                                index < state.seatsmodel!.message!.busDetailsVm!.totalRow!;
                                                index++)
                                              Row(
                                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                ...
                                                children: [
                                                  for (int i = 0;
                                                      i < state.seatsmodel!.message!.busDetailsVm!.totalColumn!;
                                                      i++)
                                                    Expanded(
                                                      child: state.seatsmodel!.message!.busDetailsVm!.rowList![index]
                                                                  .seats[i].isAvailable ==
                                                              true
                                                          ? InkWell(
                                                              onTap: () {
                                                                if (state.seatsmodel!.message!.busDetailsVm!
                                                                            .rowList![index].seats[i].isReserved ==
                                                                        false ||
                                                                    selectedseats.contains(state
                                                                            .seatsmodel!
                                                                            .message!
                                                                            .busDetailsVm!
                                                                            .rowList![index]
                                                                            .seats[i]
                                                                            .seatBusID) &&
                                                                        state.seatsmodel!.message!.busDetailsVm!
                                                                                .rowList![index].seats[i].isAvailable ==
                                                                            true) {
                                                                  if (selectedseats.contains(state
                                                                    ...
                                                                    if (reservedseats.isNotEmpty &&
                                                                        transportationListModel!
                                                                                .message!.sameSeatCount ==
                                                                            true) {
                                                                      if (reservedseats.every(
                                                                        (element) {
                                                                          return element.seatsnumber.length ==
                                                              ...
                                                              child: Container(
                                                                ...
                                                                height: (MediaQuery.of(context).size.height * 0.49) /
                                                                    state.seatsmodel!.message!.busDetailsVm!.totalRow!,
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Container(
                                      child:
                                          transportationListModel?.message!.isRequired == true && reservedseats.isEmpty
                                              ? Container(
                                                  width: 27,
                                                  height: 27,
                                                  decoration: BoxDecoration(
                                  ...
                                  children: [
                                    ...
                                    transportSuggestedList?.isEmpty == true
                                        ...
                                        : Container(
                                            ...
                                            alignment: Alignment.centerLeft,
                                            child: Text(
                                              LanguageClass.isEnglish ? 'Suggested Rides' : "الرحلات المقترحة",
                                              style: fontStyle(
                                                  color: Colors.black,
                                                  fontSize: 25.sp,
                                                  fontFamily: FontFamily.medium,
                                        ...
                                        : Expanded(
                                            child: Scrollbar(
                                              ...
                                              child: ListView.builder(
                                                ...
                                                itemCount: transportSuggestedList!.length,
                                                shrinkWrap: true,
                                                padding: EdgeInsets.symmetric(horizontal: 20),
                                                physics: ScrollPhysics(),
                                                itemBuilder: (BuildContext context, int index) {
                                                  return transportSuggestedList![index].isActive!
                                                      ? Container(
                                                          ...
                                                          child: Row(
                                                            ...
                                                            children: [
                                                              InkWell(
                                                                onTap: () {
                                                                  ...
                                                                  _umraBloc.add(GetSeatsEvent(
                                                                      tripId: transportSuggestedList![index].tripId));
                                                                },
                                                                child: Row(
                                                                  children: [
                                                                    Container(
                                                                    ...
                                                                    reservedseats
                                                                        ...
                                                                        : SizedBox(),
                                                                    Text(
                                                                      LanguageClass.isEnglish ? 'Seats' : 'مقاعد',
                                                                      style: fontStyle(
                                                                          color: Colors.black,
                                                                          fontSize: 17.sp,
                                                                          fontWeight: FontWeight.normal,
                                                              ...
                                                              16.horizontalSpace,
                                                              Text(
                                                                transportSuggestedList![index].tripDate!,
                                                                style: fontStyle(
                                                                    color: Colors.black,
                                                                    fontSize: 17.sp,
                                                                    fontWeight: FontWeight.w500,
                                    ...
                                    InkWell(
                                      onTap: () {
                                        ...
                                        showDialog(
                                          ...
                                          if (UmraDetails.swatransportList!.isNotEmpty == true &&
                                              UmraDetails.Swabusreservedseats.isNotEmpty) {
                                            transportList!.addAll(UmraDetails.swatransportList!);

                                            reservedseats.addAll(UmraDetails.Swabusreservedseats);
                                          }
                                      ...
                                      child: Container(
                                        ...
                                        child: InkWell(
                                          child: Container(
                                            ...
                                            alignment: Alignment.center,
                                            decoration: BoxDecoration(
                                                color: Color(0xffFF5D4B), borderRadius: BorderRadius.circular(6)),
                                            child: Row(
                                              mainAxisAlignment: MainAxisAlignment.center,
                                              crossAxisAlignment: CrossAxisAlignment.center,
                                              children: [
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  InkWell(
                                    onTap: transportationListModel?.message!.isRequired == true && reservedseats.isEmpty
                                        ? () {}
                                        : () {
                                            UmraDetails.transportList = [];
                                            UmraDetails.reservedseats = [];
                                            UmraDetails.transportList!.addAll(transportList!);
                                            UmraDetails.reservedseats.addAll(reservedseats);

                                            if (transportationListModel!.message!.sameSeatCount == true &&
                                                reservedseats.isNotEmpty) {
                                              int numberbookedseats = reservedseats.first.seatsnumber.length;

                                              if (transportList!.length == reservedseats.length &&
                                                  reservedseats.every(
                                                    (element) {
                                                      return element.seatsnumber.length == numberbookedseats;
                                                    },
                                                  )) {
                                                Navigator.push(
                                                    context,
                                                    MaterialPageRoute(
                                                        builder: (context) => AccomidationScreen(
                                    ...
                                    child: Container(
                                      ...
                                      alignment: Alignment.center,
                                      decoration: BoxDecoration(
                                          color: transportationListModel?.message!.isRequired == true &&
  ...
  Widget _buildTransporatinList() {
    return transportList?.isEmpty == true
        ? Center(
            child: Text(
              LanguageClass.isEnglish ? 'No Transportation' : 'لا يوجد انتقالات',
        ...
        : Flexible(
            child: ListView.builder(
              ...
              physics: ScrollPhysics(),
              itemBuilder: (BuildContext context, int index) {
                return transportList![index].isActive!
                    ? Container(
                        margin: EdgeInsets.only(top: 15),
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          ...
                          children: [
                            InkWell(
                              onTap: () {
                                isSuggested = false;
                                inn = index;
                                _umraBloc.add(GetSeatsEvent(tripId: transportList![index].tripId));
                              },
                              child: Row(
                                children: [
                                  Container(
                                        ...
                                        color: reservedseats
                                                .firstWhere((element) {
                                                  return element.tripid == transportList![index].tripId;
                                                    ...
                                                    orElse: () =>
                                                        TransportationsSeats(seatsnumber: [], totalprice: 0, tripid: 0))
                                                .seatsnumber
                                                .isNotEmpty
                                            ? AppColors.umragold
                                            : Color(0xff707070),
                                        borderRadius: BorderRadius.circular(4)),
                                    child: SvgPicture.asset('assets/images/seats.svg'),
                                  ),
                                  10.horizontalSpace,
                                  reservedseats
                                          .firstWhere((element) {
