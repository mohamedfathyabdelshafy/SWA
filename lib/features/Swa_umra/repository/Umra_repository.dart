...
class UmraRepos {
  ...
  Future getseats({int? tripid}) async {
    ...
    print("TikTik Get seat response ${decode}");
  ...
  Future addReservationMyWallet({
    ...
    String ddMMyyDate = convertDateFormat(UmraDetails.tripdate);
    print("TIKTIK DATE: ${UmraDetails.dateTypeID} $ddMMyyDate");
    ...
    log("TIK TIK ADD UMRAH BODY: $data");

    var response = await apiConsumer.post(
      EndPoints.reservationUmra,
      body: data,
    );
    // log('body 1' + data.toString());

    log('Tik Tik ReservationResponse ' + response.body);
  ...
  Future Edditreservation({
    ...
    log('body 2' + datapayment.toString());
  ...
  Future getTransportation(
    ...
    var countryid =
        ...
        3;
    print(
        "RES FOR ${EndPoints.baseUrl}TripUmra/GetTransportationList?tripUmrahID=$tripUmrahID&dateTypeID=${UmraDetails.dateTypeID}&toCurrency=${Routes.curruncy}&countryID=$countryid${reservationID != null ? '&umrahReservationID=$reservationID' : ''}");
  ...
  Future getbookedumraticket() async {
    ...
    log("TIK TIK UMRAH TICKET: ${linesModel.message!.result!.bookingList![0].transportation!.fromDate}");
    return linesModel;
  }
}
