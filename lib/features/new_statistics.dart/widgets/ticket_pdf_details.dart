...
class Ticketpdfdetails extends StatelessWidget {
  Message? ticket;

  List<Transactions>? reservations;

  Ticketpdfdetails(this.ticket, this.reservations, {Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          LanguageClass.isEnglish ? 'Ticket Details' : 'تفاصيل التذكرة',
        ),
      ),
      body: PdfPreview(
                ...
                child: pw.Container(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Text(
                        isAr ? 'سجل العمليات:' : 'Transaction Log :',
                        style: pw.TextStyle(
  ...
  List<List<pw.Widget>> _createCityStationTableData(
    ...
    for (int i = 0; i < cities.length; i += 2) {
      ...
      final sortedStations1 = city1.lineStationList!.toList()
        ..sort((a, b) => (a.orderIndex ?? 0).compareTo(b.orderIndex ?? 0));
      final stationsText1 = sortedStations1.map((s) => '${s.station!.name}').join(isAr ? ' - ' : ' - ');
      rowWidgets.add(
        pw.RichText(
          text: pw.TextSpan(
      ...
      if (i + 1 < cities.length) {
        ...
        final stationsText2 = sortedStations2.map((s) => '${s.station!.name}').join(isAr ? ' - ' : ' - ');
        rowWidgets.add(
          pw.RichText(
            text: pw.TextSpan(
  ...
  List<List<pw.Widget>> _createtransactions(
    List<Transactions> reservations,
    bool isAr,
    pw.Font ttf,
    pw.Font ttfBold,
    ...
    for (int i = 0; i < reservations.length; i++) {
      ...
      rowWidgets.add(
        pw.Text(
          "${reservations[i].transactionType}",
      ...
      rowWidgets.add(
        pw.Text(
          "${reservations[i].price} ${reservations[i].currencySymbole}",
          style: pw.TextStyle(font: ttf, fontSize: 10),
        ),
      );
      rowWidgets.add(
        pw.Text(
          "${intl.DateFormat(
            'yyyy-MM-dd hh:mm a',
          ).format(reservations[i].transactionDate!)} ",
          style: pw.TextStyle(font: ttf, fontSize: 10),
        ),
      );
